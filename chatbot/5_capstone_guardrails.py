...
from multi_nutrition_agent import breakfast_advisor_guarded

@cl.on_chat_start
async def on_chat_start():
    session = SQLiteSession("chat_agent_session")
    cl.user_session.set("chat_agent_session",session)


@cl.on_message
async def on_message(message: cl.Message):
    session = cl.user_session.get("chat_agent_session")

    result = Runner.run_streamed(
        breakfast_advisor_guarded,
        message.content,
        session= session
    )
